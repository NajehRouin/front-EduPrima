...
import ModifierSpecialite from "../../components/ModifierSpecialite"; // Import the new modal
...
function GestionSpecialite() {
  ...
  const [showModalModifier, setShowModalModifier] = useState(false); // State for edit modal
  const [selectedSpecialite, setSelectedSpecialite] = useState(null); // Store selected specialty
  const rowsPerPage = 3;
  ...
  const filteredSpecilaite = specialites.filter((c) =>
  ...
  const handleEditClick = (specialite) => {
  ...
  return (
    <div className="p-6 bg-white shadow-md rounded-xl max-w-6xl mx-auto mt-10">
              ...
              paginatedData?.map((sp) => (
                <tr key={sp._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 flex gap-4 items-center">
                    <button
                      onClick={() => handleEditClick(sp)} // Open edit modal
      ...
      {showModalModifier && selectedSpecialite && (
        <ModifierSpecialite
