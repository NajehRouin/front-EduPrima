...
function ModifierDepot({ show, onClose, fetchData, idDepot, activite }) {
  ...
  const handleDepotSubmit = async () => {
    ...
    formData.append("idDepot", idDepot);
    formData.append("fichier", selectedFile);

    try {
      const response = await fetch(Api.ModifierDepot.url, {
        method: Api.ModifierDepot.method,
        credentials: "include",
        body: formData,
      });
    ...
    show && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md">
          <h3 className="text-lg font-semibold mb-4">Modifier un dépôt</h3>
          <p className="mb-2">
